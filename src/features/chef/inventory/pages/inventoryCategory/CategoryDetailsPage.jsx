...
export default function CategoryDetailsPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-10">
      <div>
        <h1 className="text-3xl font-bold text-foreground tracking-tight">Dairy Category Item Details</h1>
        <p className="text-muted-foreground mt-1 text-sm">View and update all dairy category item information</p>
      </div>
          ...
          <Table>
            <TableHeader className="bg-muted/10 border-b border-border">
              <TableRow className="hover:bg-transparent">
                <TableHead className="text-xs font-bold text-muted-foreground uppercase tracking-wider h-14 pl-6">Item Details</TableHead>
                <TableHead className="text-xs font-bold text-muted-foreground uppercase tracking-wider h-14">SKU Identifier</TableHead>
                <TableHead className="text-xs font-bold text-muted-foreground uppercase tracking-wider text-right h-14 pr-6">Actions</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {items.map((item, index) => (
                <TableRow key={index} className="border-border hover:bg-muted/30 transition-colors">
                  <TableCell className="pl-6 py-5">
                     <span className="font-bold text-[15px] text-foreground">{item.name}</span>
                  </TableCell>
                  ...
                  <TableCell className="py-5 text-right pr-6">
                    <Link to={`/chef/inventory/category/1/item/${item.id}/edit`} className="inline-flex text-muted-foreground hover:text-foreground transition-colors p-2 rounded-lg hover:bg-muted">
                      <Edit2 className="w-4 h-4" />
                    </Link>

                    <Link to={`/chef/inventory/category/1/item/${item.id}/delete`} className="inline-flex text-muted-foreground hover:text-foreground transition-colors p-2 rounded-lg hover:bg-muted">
