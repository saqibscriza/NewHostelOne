...
const Topbar = ({ onMenuClick }) => {
  const { role, userName } = useAuth(); // get role from context
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="text-right min-w-[120px]">
          ...
          <p className="text-xs text-muted-foreground uppercase tracking-wide whitespace-nowrap">
            {role === "admin" ? "ADMIN" : role === "superadmin" ? "SUPER ADMIN" : role === "student" ? "STUDENT" : role?.toUpperCase() || "USER"}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="h-8 w-8 bg-muted rounded-full flex items-center justify-center overflow-hidden">
             <img src={`https://api.dicebear.com/7.x/notionists/svg?seed=${userName || 'Rakesh'}&backgroundColor=0f172a`} alt="Profile" className="w-full h-full object-cover" />
          </div>
        </div>
      </div>
    </header>
