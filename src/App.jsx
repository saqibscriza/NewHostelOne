...
export default function AppRoutes() {
  ...
  useEffect(() => {
    console.log("Registering onMessage listener with messaging instance:", messaging);
    const unsubscribe = onMessage(messaging, (payload) => {
      console.log("Foreground message received! Payload:", payload);
      setFireBaseValue(payload.notification);
      setFireBaseId({
        messageId: payload?.data?.messageId,
        trigger: Date.now(),
  ...
  useEffect(() => {
    const initFCM = async () => {
      ...
      if (token && jwtToken) {
        try {
          await registerApiWithFCMToek({
            token: token,
            deviceType: "WEB"
          });
          console.log("FCM Token successfully registered with backend from App.jsx");
        } catch (err) {
          console.error("Failed to register FCM token from App.jsx", err);
        }
      }
    };
